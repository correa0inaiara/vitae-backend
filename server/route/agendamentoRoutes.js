...
router.get('/usuario/:id', async function (req, res, next) {
	try {
		if (req.headers.token) {
			...
			if (decode) {
				const usuarioId = req.params.id;
				const agendamento = await agendamentoService.getAgendamentoByUsuario(usuarioId);
				res.status(200).json(agendamento);
			} else res.status(401).json({message: 'Falha na autenticação.'});
		} else  res.status(401).json({message: 'Usuário não pode ser autenticado.'});
	} catch (error) {
...
router.post('/:id', async function (req, res, next) {
	try {
		...
		const agendamento = await agendamentoService.saveAgendamento(id, candidatoSelecionadoId, body);
	...
	try {
		if (req.headers.token) {
			...
			if (decode) {
				const id = req.params.id;
				const body = req.body;
				const agendamento = await agendamentoService.updateAgendamento(id, body);
				res.status(200).json(agendamento);
			} else res.status(401).json({message: 'Falha na autenticação.'});
		} else  res.status(401).json({message: 'Usuário não pode ser autenticado.'});
	} catch (error) {
